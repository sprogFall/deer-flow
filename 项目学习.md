...
[`MemoryManager`](backend/packages/harness/deerflow/agents/memory/manager.py) 是后端无关的 Pydantic/ABC 契约。核心方法：

```text
add(thread_id, messages, agent_name, user_id, trace_id)
get_context(user_id, agent_name, thread_id) -> injection text
search(query, top_k, user_id, agent_name, category) -> facts（可选能力）
get_memory / clear_memory / import_memory / create_fact / update_fact / delete_fact
shutdown_flush(timeout)
```

所有 backend 必须实现 `add` 和 `get_context`。如果要支持 `memory.mode: tool`，必须覆写 `search()` 且声明 `supports_search=True`，基类会在实例化时校验二者是否一致。

`get_memory_manager()` 是按配置解析的单例 factory：`manager_class` 可以是内置名称（`deermem`、`mem0`、`openviking`、`noop`），也可以是 dotted import path。Host 只依赖契约，不依赖具体 facts 存储模型。

### 7.2 两种运行模式

#### Middleware 模式

每次 agent run 的 `MemoryMiddleware.after_agent/aafter_agent` 把对话交给 manager。下一轮调用前，middleware 通过 `get_context()` 把长期记忆包进 `<memory>...</memory>` 系统上下文。
...
模型自己调用 `memory_search`、`memory_add`、`memory_update`、`memory_delete`。自动注入通常只放 user/history 全局摘要，agent facts 由查询结果提供，避免同一事实同时出现在 system prompt 和 tool 返回值中。

Tool 模式把写入主动权交给模型，灵活但依赖模型是否正确调用工具；因此 backend 可以声明 `requires_passive_writes_in_tool_mode=True`，保留被动对话抽取，同时提供 query-aware search。

### 7.3 DeerMem：默认的本地事实记忆

DeerMem 的代码在 [`agents/memory/backends/deermem`](backend/packages/harness/deerflow/agents/memory/backends/deermem)。它把“抽取、存储、检索、并发和兼容性”都放在本地 backend 内。

#### 写入链路

```text
MemoryMiddleware
  -> filter_messages_for_memory（用户消息 + 最终 AI 答案）
  -> filter_trivial（去掉纯确认等无信息轮次）
  -> detect_signals（preference/correction/goal/decision...）
  -> MemoryUpdateQueue（去抖、合并、背压）
  -> memory_update LLM prompt
  -> scope/durability/authority write gate
  -> Updater 原子写入
```

队列是 daemon worker，但 Gateway 正常关闭时会调用 `shutdown_flush(timeout)`，尽量把仍在 debounce buffer 中的更新刷完，避免滚动发布或 SIGTERM 丢失最近一轮记忆。

#### v2 存储布局

DeerMem 把共享摘要和 agent facts 分开：

```text
.deer-flow/<memory root>/users/<user_id>/
...
没有显式 `agent_name` 的旧调用会落入保留桶 `__default__`。用户级 `user/history` 是跨 agent 的共享上下文；facts 按 agent 隔离。旧版把 facts 塞在 JSON 中，首次正常读取或 CLI migration 会迁移到 Markdown，并保留 `.v1.bak` 保护源数据。

#### 检索链路

默认使用可重建的 SQLite FTS5/BM25 derived index；索引损坏可自动重建，未安装中文 tokenizer 时退回大小写不敏感 substring 搜索。索引不是 canonical data，真正事实仍在 Markdown 文件中，因此可以删掉 `.retrieval/` 后重建。
...
`max_injection_tokens` 是硬上限；可以使用字符计数或 tiktoken。DeerMem 先保证配置的关键类别，再按预算压缩其余事实。注入文本由 backend 生成，middleware 只负责 gate 和 `<memory>` 包装，这保证其它 backend 不必伪装成 DeerMem 的数据格式。

### 7.4 mem0：远程托管、无本地状态

[`Mem0Manager`](backend/packages/harness/deerflow/agents/memory/backends/mem0/mem0_manager.py) 通过 HTTP 调用 mem0 Platform 或兼容服务：

- `(user_id, agent_name)` 映射到 mem0 的 `user_id/agent_id`，thread 映射到 `run_id`。
- `add` 只做消息过滤和角色映射，抽取、去重、存储由 mem0 服务端完成。
- `get_context` 查询最近 memories，`search` 提供 query-aware 结果，因此支持 tool 模式。
- `read_policy` 和 `write_policy` 可以选择 fail-open 或 log-and-drop：远程记忆故障不一定阻断主 agent。
- 不保留本地 queue/watermark/cache，天然适合多 worker，但依赖外部服务的可用性和网络延迟。

### 7.5 OpenViking：远程 Session + 检索

[`OpenVikingMemoryManager`](backend/packages/harness/deerflow/agents/memory/backends/openviking/openviking_manager.py) 通过 HTTP 把过滤后的会话提交给 OpenViking Sessions，并将远程 search hits 映射为 DeerFlow facts：

- 当前实现主要支持 middleware 模式，构造时会拒绝不支持的 mode。
- `get_context` 使用配置的 `injection_query` 做检索，而不是把整个远程库无条件搬回 prompt。
- 有 session scope、并发 operation 计数、close/health 生命周期和失败策略。
- 适合把向量/层级记忆交给专用服务；代价是部署多一个系统，且远程检索失败需要明确 fail-open/fail-closed 语义。

### 7.6 noop 与自定义 backend

`noop` 满足契约但不保存、不注入，用于关闭记忆或测试。自定义 backend 只需：

1. 继承 `MemoryManager`。
2. 实现 `add` 和 `get_context`。
3. 若支持 tool 模式，实现 `search` 并设 `supports_search=True`。
4. 在 backend 包暴露 `MANAGER_CLASS`，或配置完整 import path。
5. 处理 close、失败策略、user/agent scope 和 token budget。

真正稳定的扩展点是“契约 + host hook”，不是复制 DeerMem 的内部类。backend 可以不使用 facts，甚至可以是远程向量库，只要返回 injection text 和兼容的 search result。
